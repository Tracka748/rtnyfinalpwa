...
export function TonightSection({ events }: { events: Event[] }) {
  if (events.length === 0) return null

  return (
    <section className="pt-6 pb-4 mb-2 mx-auto max-w-[1200px]">
      <div className="flex items-center justify-between mb-4 px-4">
        <h2 className="font-header text-xl font-bold text-white">🌙 Tonight</h2>
      </div>
      <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide snap-x px-4">
        {events.map((event) => (
          <Link
            ...
            className="group shrink-0 snap-start w-52 rounded-xl overflow-hidden border border-border bg-surface hover:border-accent-primary/50 transition-all"
          >
